...
import { getFleetUtilization } from "@/lib/dashboard/utilization";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await getFleetUtilization(parseResult.data);
    return Response.json(data);
  } catch (err: unknown) {
    console.error("[GET /api/dashboard/charts/fleet-utilization]", err);
    return Response.json(
      { error: err instanceof Error ? err.message : "Failed to fetch fleet utilization" },
      { status: 500 }
    );
  }
}
