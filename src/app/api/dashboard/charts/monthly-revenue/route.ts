...
import { getMonthlyRevenue } from "@/lib/dashboard/revenue";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await getMonthlyRevenue(parseResult.data);
    return Response.json(data);
  } catch (err: unknown) {
    console.error("[GET /api/dashboard/charts/monthly-revenue]", err);
    return Response.json(
      { error: err instanceof Error ? err.message : "Failed to fetch monthly revenue" },
      { status: 500 }
    );
  }
}
