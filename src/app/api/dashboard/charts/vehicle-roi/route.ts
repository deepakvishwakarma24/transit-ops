...
import { getVehicleROI } from "@/lib/dashboard/costs";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await getVehicleROI(parseResult.data);
    return Response.json(data);
  } catch (err: unknown) {
    console.error("[GET /api/dashboard/charts/vehicle-roi]", err);
    return Response.json(
      { error: err instanceof Error ? err.message : "Failed to fetch vehicle ROI" },
      { status: 500 }
    );
  }
}
